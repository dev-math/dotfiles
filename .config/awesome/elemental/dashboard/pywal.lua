...
-- Create the widget
dashboard = wibox({visible = false, ontop = true, type = "dock", screen = screen.focused})
...
user_picture_container.forced_width = dpi(150)
...
local calendar_box = create_boxed_widget(calendar, dpi(350), dpi(400), x.background)
-- local calendar_box = create_boxed_widget(calendar, 380, 540, x.color0)

-- Time widget
-- 12 h format %l %M %P
local hours = wibox.widget.textclock("%H ")
hours.font = "SF UI Display bold 30"
hours.align = "center"
hours.valign = "center"
local minutes = wibox.widget.textclock("<span foreground='" .. x.color6 .."'>  %M </span>")
minutes.font = "SF UI Display 30"
...
-- Date
local day_of_the_week = wibox.widget.textclock("<span foreground='" .. x.color6 .."'>%A </span>")
day_of_the_week.font = "SF UI Display italic 20"
day_of_the_week.fg = x.color0
day_of_the_week.align = "center"
day_of_the_week.valign = "center"

local day_of_the_month = wibox.widget.textclock("<span foreground='" .. x.color7 .."'>%d</span>")
day_of_the_month.font = "SF UI Display bold 30"
...
local function create_bookmark(name, path, color, hover_color)
    ...
    bookmark.font = "SF UI Display bold 16"
...
local bookmarks = wibox.widget {
    create_bookmark("home", os.getenv("HOME"), x.color3, x.color9),
    create_bookmark("documents", user.dirs.documents, x.color6, x.color12),
    create_bookmark("downloads", user.dirs.downloads, x.color2, x.color8),
    --create_bookmark("music", user.dirs.music, x.color6, x.color12),
    --create_bookmark("videos", user.dirs.videos, x.color6, x.color12),
    create_bookmark("pictures", user.dirs.pictures, x.color4, x.color9),
    create_bookmark("screenshots", user.dirs.screenshots, x.color3, x.color9),
    --create_bookmark("wallpapers", user.dirs.wallpapers, x.color5, x.color10),
    spacing = dpi(10),
    layout = wibox.layout.fixed.vertical
}
...
local function create_url(name, path, original_color, hover_color)
    ...
    url.font = "SF UI Display bold 16"
    ...
    helpers.add_hover_cursor(url, "hand2")

    return url
end

local urls = wibox.widget {
    create_url("TWITTER", "twitter.com", x.color4, x.color10),
    create_url("YT MUSIC", "music.youtube.com", x.color4, x.color10),
    create_url("SAVAGE DIVINITY", "ruffwriterblog.wordpress.com", x.color4, x.color10),
    --create_url("REDDIT", "reddit.com", x.color4, x.color10),
    --create_url("GITHUB", "github.com/elenapan", x.color4, x.color10),
    spacing = dpi(10),
    layout = wibox.layout.fixed.vertical
}
...
local brightness_icon = wibox.widget.textbox("<span foreground='" .. x.color5 .."'> </span>")
brightness_icon.font = "Font Awesome 5 Free 30"
...
helpers.add_hover_cursor(brightness_box, "hand2")
...
local corona_box = create_boxed_widget(corona, dpi(250), dpi(170), x.background)
