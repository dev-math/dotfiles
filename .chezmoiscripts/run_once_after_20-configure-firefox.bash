...
# Start firefox in headless mode in order to create config directory for it.
mkdir -p $HOME/.mozilla
firefox --headless >/dev/null 2>&1 &
sleep 3
echo "kill firefox before continuing"
# pkill -f firefox

# Find the profile folder.
LIBREW_CONFIG_DIR="$HOME/.mozilla/firefox"
LIBREW_PROF_NAME="$(sed -n "/Default=.*.default/ s/.*=//p" "$LIBREW_CONFIG_DIR/profiles.ini")"
